...
public class ProcessService {
    ...
    public Page<Biomaterial> findInputBiomaterialsForProcess(final Process process, final Pageable pageable) {
    ...
    public Page<Biomaterial> findOutputBiomaterialsForProcess(final Process process, final Pageable pageable) {
    ...
    public Process addProcessToSubmissionEnvelope(final SubmissionEnvelope submissionEnvelope, final Process process) {
    ...
    public Process addInputBiomaterialToProcess(final Process process, final Biomaterial biomaterial) {
    ...
    private File determineTargetFile(final SubmissionEnvelope submissionEnvelope, final File file) {
        final List<File> persistentFiles = fileRepository
    ...
    public Process addInputBundleManifest(final Process analysisProcess, final BundleReference bundleReference) {
        for (final String bundleUuid : bundleReference.getBundleUuids()) {
            final Optional<BundleManifest> maybeBundleManifest = getBundleManifestRepository().findTopByBundleUuidOrderByBundleVersionDesc(bundleUuid);
    ...
    public Page<Process> findProcessesByInputBundleUuid(final UUID bundleUuid, final Pageable pageable) {
        final Optional<BundleManifest> maybeBundleManifest = bundleManifestRepository.findTopByBundleUuidOrderByBundleVersionDesc(bundleUuid.toString());
        return maybeBundleManifest.map(bundleManifest -> processRepository.findByInputBundleManifestsContaining(bundleManifest, pageable))
                .orElseThrow(() -> new ResourceNotFoundException(String.format("Bundle with UUID %s not found", bundleUuid.toString())));
    }
    ...
    public Process resolveBundleReferencesForProcess(final Process analysis, final BundleReference bundleReference) {
        for (final String bundleUuid : bundleReference.getBundleUuids()) {
            final Optional<BundleManifest> maybeBundleManifest = getBundleManifestRepository().findTopByBundleUuidOrderByBundleVersionDesc(bundleUuid);
