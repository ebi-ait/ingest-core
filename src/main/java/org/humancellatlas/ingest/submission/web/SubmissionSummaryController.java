...
public class SubmissionSummaryController {
    ...
    public SubmissionSummary submissionSummary(@PathVariable("sub_id") SubmissionEnvelope submissionEnvelope) {
        ...
        // This allows us to filter by graph invalid entities until we consolidate the graphValidationState into
        // validationState
        long graphInvalidBiomaterials = biomaterialRepository.countBySubmissionEnvelopeAndCountWithGraphValidationErrors(submissionEnvelope.getId());

        long invalidFiles = fileRepository.countBySubmissionEnvelopeAndValidationState(submissionEnvelope, ValidationState.INVALID);
        long graphInvalidFiles = fileRepository.countBySubmissionEnvelopeAndCountWithGraphValidationErrors(submissionEnvelope.getId());
        ...
        long graphInvalidProcesses = processRepository.countBySubmissionEnvelopeAndCountWithGraphValidationErrors(submissionEnvelope.getId());
        ...
        long graphInvalidProtocols = protocolRepository.countBySubmissionEnvelopeAndCountWithGraphValidationErrors(submissionEnvelope.getId());


        long totalInvalid = invalidBiomaterials + graphInvalidBiomaterials
                + (fileMetadataErrors + missingFiles + fileErrors + graphInvalidFiles)
                + invalidProcesses + graphInvalidProcesses
                + invalidProtocols + graphInvalidProtocols;
    ...
    public class SubmissionSummary {
        ...
        private Long totalFiles, invalidFiles, graphInvalidFiles, fileMetadataErrors, missingFiles, fileErrors;
