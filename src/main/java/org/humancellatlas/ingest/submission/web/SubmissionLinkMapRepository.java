...
public class SubmissionLinkMapRepository {
    ...
    List<ProcessAndInputBiomaterials> findProcessInputBiomaterials(SubmissionEnvelope submissionEnvelope) {
        String entity_type = "biomaterial";
        Aggregation agg = buildAggregationQueryForProcessInputs(submissionEnvelope, entity_type, "inputBiomaterials");
        AggregationResults<ProcessAndInputBiomaterials> processAndInputBiomaterials = mongoTemplate.aggregate(agg,
                entity_type, ProcessAndInputBiomaterials.class
        );
        return processAndInputBiomaterials.getMappedResults();
    }

    private static Aggregation buildAggregationQueryForProcessInputs(SubmissionEnvelope submissionEnvelope, String entity_type, String inputBiomaterials) {
        return newAggregation(
                ...
                project(UNDERSCORE_ID)
                        ...
                        .as("process_id"),
                project("process_id")
                        .and(UNDERSCORE_ID).as(entity_type + "_id"),
                group("$process_id.v")
    ...
    List<ProcessAndInputFiles> findProcessInputFiles(SubmissionEnvelope submissionEnvelope) {
        ...
        Aggregation agg = buildAggregationQueryForProcessInputs(submissionEnvelope, entity_type, "inputFiles");
        AggregationResults<ProcessAndInputFiles> processAndInputFiles = mongoTemplate.aggregate(agg,
                entity_type, ProcessAndInputFiles.class
        );
        return processAndInputFiles.getMappedResults();
    }

    List<EntityWithInputsAndDerivedBy> findLinkedProcessesByEntityTypeAndSubmission(SubmissionEnvelope submissionEnvelope, String entity_type) {
        List<String> jsonStages = List.of(
                "    {\n" +
                "        $project: {\n" +
