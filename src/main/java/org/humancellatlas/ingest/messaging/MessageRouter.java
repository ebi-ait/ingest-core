...
public class MessageRouter {
    ...
    public boolean sendExportJobQueuedEvent(ExportJob exportJob, Project project) {
        ExportJobMessage exportJobMessage = new ExportJobMessage(exportJob.getId(),
                linkGenerator.createCallback(exportJob.getClass(), exportJob.getId()),
                exportJob.getSubmission().getUuid().toString(),
                project.getUuid().toString());

        this.messageSender.queueNewExportMessage(EXPORTER_EXCHANGE,
                EXPORT_JOB_QUEUED,
                exportJobMessage,
                System.currentTimeMillis()
        );
