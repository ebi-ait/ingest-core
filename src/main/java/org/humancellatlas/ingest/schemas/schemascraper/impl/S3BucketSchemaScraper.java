 ...
 *
 * Scrapes schemas from an s3 bucket's default XML file-list page
 *
 */
@Service
public class S3BucketSchemaScraper implements SchemaScraper {
    ...
    public Collection<URI> getAllSchemaURIs(URI schemaBucketLocation) {
        String bucketListingXmlString = this.restTemplate.getForObject(schemaBucketLocation, String.class);
        try {
            ListBucketResult listBucketResult = xmlMapper.readValue(bucketListingXmlString, ListBucketResult.class);
            return listBucketResult.contents.stream()
                                            .map(content -> URI.create(content.getKey()))
                                            .collect(Collectors.toList());
        } catch (IOException e) {
            throw new SchemaScrapeException("Failed to parse schema bucket xml at URL: " + schemaBucketLocation, e);
        }
    }
