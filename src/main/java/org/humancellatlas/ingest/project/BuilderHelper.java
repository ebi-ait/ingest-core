...
public class BuilderHelper<T, B> {
    ...
    public void copyFieldsFromBuilder(T target, List<String> ignoreFieldsList) {
        ...
        Arrays.stream(builderInstance.getClass()
                        .getDeclaredFields())
                .filter(f -> !ignoreFieldsList.contains(f.getName()))
    ...
    private static <T> Method getMethod(T target, String fieldName, Field projectField) throws NoSuchMethodException {
