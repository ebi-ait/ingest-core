...
public class RowLevelFilterSecurityAspect {
    ...
    @Around("repositoryFindFunctions() || repositoryInheritedFindFunctions()")
    ...
    class RowlevelSecurityAdviceHelper {
        ...
        public RowlevelSecurityAdviceHelper(ProceedingJoinPoint joinPoint) {
            ...
            this.rowLevelFilterSecurity = method.getClass().getAnnotation(RowLevelFilterSecurity.class);
        ...
        private Object filterOptional(Optional<?> queryResult) {
            ...
            if(Arrays.stream(rowLevelFilterSecurity.ignoreClasses()).anyMatch(queryResult.get().getClass()::isInstance)) {
                return queryResult;
            }
            List<String> variableNames = buildVariableNames(method);
            List<Object> variableValues = buildVariableValues((File)queryResult.get(), authentication);
            if (spelHelper.parseExpression(variableNames, variableValues, rowLevelFilterSecurity.value())){
                return queryResult;
            }
