...
import uk.ac.ebi.subs.ingest.messaging.model.MessageProtocol;
import uk.ac.ebi.subs.ingest.messaging.model.MetadataDocumentMessage;
import uk.ac.ebi.subs.ingest.messaging.model.SpreadsheetGenerationMessage;
...
public class MessageSender {
  ...
  public void queueValidationMessage(
      String exchange, String routingKey, MetadataDocumentMessage payload, long intendedSendTime) {
    MessageBuffer.VALIDATION.queueAmqpMessage(exchange, routingKey, payload, intendedSendTime);
  }

  public void queueGraphValidationMessage(
      String exchange, String routingKey, Object payload, long intendedSendTime) {
    MessageBuffer.GRAPH_VALIDATION.queueAmqpMessage(
        exchange, routingKey, payload, intendedSendTime);
  }

  public void queueNewExportMessage(
      String exchange, String routingKey, Object payload, long intendedSendTime) {
    MessageBuffer.EXPORT.queueAmqpMessage(exchange, routingKey, payload, intendedSendTime);
  }

  public void queueStateTrackingMessage(
      String exchange, String routingKey, Object payload, long intendedSendTime) {
  ...
  public void queueUploadManagerMessage(
      String exchange,
      String routingKey,
  ...
  public void queueSpreadsheetGenerationMessage(
      String exchange,
      String routingKey,
      SpreadsheetGenerationMessage payload,
      long intendedSendTime) {
    MessageBuffer.SPREADSHEET_GENERATION.queueAmqpMessage(
        exchange, routingKey, payload, intendedSendTime);
  }

  @PostConstruct
  private void initiateSending() {
    List<MessageBuffer> amqpMessageBuffers =
        Arrays.asList(
    ...
    amqpMessageBuffers.forEach(
        buffer ->
            scheduler.scheduleWithFixedDelay(
                new AmqpHttpMixinBufferSender(buffer, new RestTemplate(), rabbitMessagingTemplate),
                0,
  ...
  static class QueuedMessage implements Delayed {
    ...
    public QueuedMessage(
        String exchange, String routingKey, Object payload, long intendedStartTime) {
    ...
    public QueuedMessage(
        URI uri, HttpMethod method, @Nullable Object payload, long intendedStartTime) {
  ...
  private enum MessageBuffer {
    ...
    @Getter private final Long delayMillis;
    ...
    void queueAmqpMessage(
        String exchange, String routingKey, Object payload, long intendedStartTime) {
      QueuedMessage message =
          new QueuedMessage(exchange, routingKey, payload, intendedStartTime + delayMillis);
      try {
    ...
    void queueHttpMessage(
        URI uri, HttpMethod method, @Nullable Object payload, long intendedStartTime) {
      QueuedMessage message =
          new QueuedMessage(uri, method, payload, intendedStartTime + delayMillis);
      try {
        messageQueue.add(message);
      } catch (IllegalStateException e) {
        LOGGER.error(String.format("Failed to queue message: %s", convertToString(message)), e);
    ...
    public Stream<QueuedMessage> takeAll() {
      Queue<QueuedMessage> drainedQueue =
          new PriorityQueue<>(Comparator.comparing(QueuedMessage::getIntendedStartTime));
      this.messageQueue.drainTo(drainedQueue);
      return Stream.generate(drainedQueue::remove).limit(drainedQueue.size());
    }
    ...
    private String convertToString(Object object) {
      try {
        ...
        LOGGER.debug(
            String.format(
                "An error in converting message object to string occurred: %s", e.getMessage()));
        return "";
      }
    }
  }
  ...
  private static class AmqpHttpMixinBufferSender implements Runnable {
    ...
    private final Logger log =
        LoggerFactory.getLogger(MessageSender.AmqpHttpMixinBufferSender.class);

    private AmqpHttpMixinBufferSender(
              ...
              message -> {
                if (message.getMessageProtocol().equals(MessageProtocol.AMQP)) {
                  messagingTemplate.convertAndSend(
                      message.exchange, message.routingKey, message.payload);
                } else {
                  try {
                    ...
                    log.error(
                        String.format(
                            "error sending HTTP %s message to uri %s with payload %s",
                            message.method, message.uri, message.payload),
                        e);
                  }
                }
              });
