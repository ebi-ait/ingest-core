...
/**
 * @see <a href="https://docs.spring.io/spring-framework/docs/2.0.x/reference/aop.html">Spring 2 AOP
 *     Docs</a>
 */
@Aspect
@Component
public class RowLevelFilterSecurityAspect {
  ...
  @Pointcut("execution(* uk.ac.ebi.subs.ingest.*.*Repository.find*(..))")
  public void repositoryFindFunctions() {}

  @Pointcut(
      "execution(* org.springframework.data.repository.PagingAndSortingRepository.find*(..)) ")
  public void repositoryInheritedFindFunctions() {}

  @Pointcut(
      "target(uk.ac.ebi.subs.ingest.file.FileRepository) "
          + "|| target(uk.ac.ebi.subs.ingest.biomaterial.BiomaterialRepository) "
          + "|| target(uk.ac.ebi.subs.ingest.process.ProcessRepository) "
          + "|| target(uk.ac.ebi.subs.ingest.protocol.ProtocolRepository) ")
  public void ingestRepositoriesAreTheTarget() {}

  @Around("(ingestRepositoriesAreTheTarget() " + "&& repositoryInheritedFindFunctions())")
  ...
  class RowlevelSecurityAdviceHelper {
    ...
    public RowlevelSecurityAdviceHelper(ProceedingJoinPoint joinPoint) {
      ...
      this.rowLevelFilterSecurity =
          Optional.ofNullable(method.getClass().getAnnotation(RowLevelFilterSecurity.class))
              .orElseGet(() -> readAnnotationFromSuperInterface(joinPoint));
      this.authentication =
    ...
    private RowLevelFilterSecurity readAnnotationFromSuperInterface(ProceedingJoinPoint joinPoint) {
      return (RowLevelFilterSecurity)
          Arrays.stream(joinPoint.getThis().getClass().getInterfaces())
              .filter(o -> o.getName().contains("ingest"))
              .flatMap(o -> Arrays.stream(o.getAnnotations()))
              .filter(
                  a -> a.annotationType().getName().equals(RowLevelFilterSecurity.class.getName()))
              .findFirst()
              .get();
    }

    private Object filterDocumentList(List<? extends MetadataDocument> documentList) {
      List<? extends MetadataDocument> retainedDocuments =
          documentList.stream()
    ...
    private List<Object> buildVariableValues(
        MetadataDocument document, Authentication authentication) {
    ...
    public Object filterResult(Object queryResult) {
      ...
      if (queryResult instanceof Page) {
        ...
        throw new IllegalArgumentException(
            "only supports filtering results of type Page, List, Optional. Type given: "
    ...
    private Object filterOptional(Optional<?> queryResult) {
      ...
      List<Object> variableValues =
          buildVariableValues((MetadataDocument) queryResult.get(), authentication);
      if (spelHelper.parseExpression(
          variableNames, variableValues, rowLevelFilterSecurity.expression())) {
        return queryResult;
      }
