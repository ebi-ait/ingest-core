...
public class GlobusService {
    ...
    public void mkdir(String absPath) {
        ...
        client(props.getTransferUrl())
                .post().uri("/operation/endpoint/{id}/mkdir", props.getCollectionId())
                ...
                .flatMap(resp -> {
                    ...
                    return resp.bodyToMono(String.class).flatMap(b ->
                            Mono.error(new RuntimeException("Globus mkdir " + resp.statusCode() + " body=" + b)));
                })
                .block();
    }
    ...
    public Map<String,Object> ls(String absPath) {
        ...
        return client(props.getTransferUrl())
                .get().uri(uri -> uri.path("/operation/endpoint/{id}/ls")
                        .queryParam("path", absPath).build(props.getCollectionId()))
                .header("Authorization","Bearer " + tok)
                .retrieve()
    ...
    private String tokenForTransfer() {
        ...
        String dataAccess = (props.getDataAccessScope() != null && !props.getDataAccessScope().isBlank())
        ...
        Map<String,Object> resp = client(props.getAuthUrl())
                .post()
                .uri("/v2/oauth2/token")
        ...
        tokens.add(resp);
        tokens.addAll((java.util.List<Map<String,Object>>) resp.getOrDefault("other_tokens", java.util.List.of()));

        String transferToken = tokens.stream()
                ...
                .orElseThrow(() -> new RuntimeException(
                        "No token for resource_server=transfer.api.globus.org. Got resource_servers=" +
                                tokens.stream().map(t -> String.valueOf(t.get("resource_server"))).collect(java.util.stream.Collectors.toList())
                ));
    ...
    public String getSubmissionId() {
        ...
        Map<?, ?> resp = client(props.getTransferUrl())
                .get()
                .uri("/submission_id")
                .header("Authorization", "Bearer " + tok)
                .retrieve()
    ...
    public Map<String, Object> submitDelete(String baseDir, List<String> relPaths, boolean recursive) {
        String tok = tokenForTransfer();
