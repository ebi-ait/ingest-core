                  ...
                  .ifPresentOrElse(
                      failedReport ->
                          this.notificationService.changeState(
                              processingNotification, NotificationState.FAILED),
                      () ->
                          this.notificationService.changeState(
                              processingNotification, NotificationState.PROCESSED));
            });
  }
        ...
        .map(
            notificationProcessor -> {
              try {
                ...
                log.warn(
                    String.format(
                        "Notification processor failed for %s on notification with ID %s",
                        notificationProcessor.getClass(), notification.getId()),
                    e);
