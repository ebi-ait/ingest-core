...
import uk.ac.ebi.subs.ingest.process.*;
...
public class ProcessController {
  ...
  private @Autowired final ValidationStateChangeService validationStateChangeService;
  private @Autowired final UriToEntityConversionService uriToEntityConversionService;
  private @Autowired final MetadataLinkingService metadataLinkingService;

  // Input and Output Resources
  @GetMapping("processes/{proc_id}/inputBiomaterials")
  ResponseEntity<?> getProcessInputBiomaterials(
      @PathVariable("proc_id") final Process process,
      final Pageable pageable,
      final PersistentEntityResourceAssembler assembler) {
    final Page<Biomaterial> inputBiomaterials =
        processService.findInputBiomaterialsForProcess(process, pageable);
  ...
  ResponseEntity<?> getProcessInputFiles(
      @PathVariable("proc_id") final Process process,
      final Pageable pageable,
      final PersistentEntityResourceAssembler assembler) {
  ...
  ResponseEntity<?> getProcessOutputBiomaterials(
      @PathVariable("proc_id") final Process process,
      final Pageable pageable,
      final PersistentEntityResourceAssembler assembler) {
    final Page<Biomaterial> outputBiomaterials =
        processService.findOutputBiomaterialsForProcess(process, pageable);
  ...
  ResponseEntity<?> getProcessOutputFiles(
      @PathVariable("proc_id") final Process process,
      final Pageable pageable,
      final PersistentEntityResourceAssembler assembler) {
  ...
  ResponseEntity<Resource<?>> addProcessToEnvelope(
    ...
    final Process entity =
        processService.addProcessToSubmissionEnvelope(submissionEnvelope, process);
  ...
  ResponseEntity<Resource<?>> linkProcessToEnvelope(
      @PathVariable("sub_id") final SubmissionEnvelope submissionEnvelope,
  ...
  @CheckAllowed(
      ...
      exception = NotAllowedDuringSubmissionStateException.class)
  ResponseEntity<Resource<?>> oldAddBundleReference(
      @PathVariable("analysis_id") final Process analysis,
      @RequestBody final BundleReference bundleReference,
      final PersistentEntityResourceAssembler assembler) {
    final Process entity =
        processService.resolveBundleReferencesForProcess(analysis, bundleReference);
    final PersistentEntityResource resource = assembler.toFullResource(entity);
    return ResponseEntity.accepted().body(resource);
  }
  ...
  @CheckAllowed(
      value = "#analysis.submissionEnvelope.isSystemEditable()",
      exception = NotAllowedDuringSubmissionStateException.class)
  ResponseEntity<Resource<?>> addInputFileReference(
      @PathVariable("analysis_id") final Process analysis,
      @RequestBody final InputFileReference inputFileReference,
      final PersistentEntityResourceAssembler assembler) {
    final Process result =
        processService.addInputFileUuidToProcess(analysis, inputFileReference.getInputFileUuid());
    final PersistentEntityResource resource = assembler.toFullResource(result);
    return ResponseEntity.accepted().body(resource);
  }
  ...
  @CheckAllowed(
      ...
      exception = NotAllowedDuringSubmissionStateException.class)
  @RequestMapping(
      path = "/processes/{id}/protocols",
      method = {PUT, POST},
      consumes = {TEXT_URI_LIST_VALUE})
  HttpEntity<?> linkProtocolsToProcess(
    ...
    List<Protocol> protocols =
        uriToEntityConversionService.convertLinks(incoming.getLinks(), Protocol.class);
    metadataLinkingService.updateLinks(
        process, protocols, "protocols", requestMethod.equals(HttpMethod.PUT));
  ...
  @CheckAllowed(
      value = "#process.submissionEnvelope.isEditable()",
      exception = NotAllowedDuringSubmissionStateException.class)
  HttpEntity<?> unlinkProtocolFromProcess(
      @PathVariable("id") final Process process,
      @PathVariable("protocolId") final Protocol protocol)
      throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
  ...
  ResponseEntity<?> findProcesessByInputBundleUuid(
      @RequestParam final String bundleUuid,
      final Pageable pageable,
      final PersistentEntityResourceAssembler resourceAssembler) {
    final Page<Process> processes =
        processService.findProcessesByInputBundleUuid(UUID.fromString(bundleUuid), pageable);
