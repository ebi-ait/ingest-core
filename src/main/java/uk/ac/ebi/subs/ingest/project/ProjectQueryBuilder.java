...
import uk.ac.ebi.subs.ingest.project.web.SearchType;
...
public class ProjectQueryBuilder {
  ...
  public static Query buildProjectsQuery(SearchFilter searchFilter) {
    ...
    addIsCriterionForAttribute(
        criteriaList, "wranglingPriority", searchFilter.getWranglingPriority());
    addIsCriterionForAttribute(criteriaList, "isInCatalogue", searchFilter.getHcaCatalogue());
    ...
    addInCriterionForAttribute(
        criteriaList, "identifyingOrganisms", searchFilter.getIdentifyingOrganism());
    addInCriterionForAttribute(
        criteriaList, "dcpReleaseNumber", searchFilter.getDcpReleaseNumber());
    ...
    if (searchFilter.getDataAccess() != null) {
      addIsCriterionForAttribute(
          criteriaList, "content.dataAccess.type", searchFilter.getDataAccess().getLabel());
    }
    ...
    addKeywordSearchCriteria(searchFilter).ifPresent(query::addCriteria);
    return query;
  }
  ...
  private static Optional<CriteriaDefinition> buildTextCriteria(SearchFilter searchFilter) {
    return Optional.ofNullable(searchFilter.getSearch())
        ...
        .map(search -> TextCriteria.forDefaultLanguage().matching(String.valueOf(search)));
  }
  ...
  private static void addIsCriterionForAttribute(
      List<Criteria> criteria_list, String attributeName, Object attributeValue) {
    Optional.ofNullable(attributeValue)
        .map(value -> Criteria.where(attributeName).is(value))
        .ifPresent(criteria_list::add);
  }

  private static void addLTECriterionForAttribute(
      List<Criteria> criteria_list, String attributeName, Integer attributeValue) {
    Optional.ofNullable(attributeValue)
        .map(value -> Criteria.where(attributeName).lte(value))
        .ifPresent(criteria_list::add);
  }

  private static void addGTECriterionForAttribute(
      List<Criteria> criteria_list, String attributeName, Integer attributeValue) {
    Optional.ofNullable(attributeValue)
  ...
  private static void addInCriterionForAttribute(
      List<Criteria> criteria_list, String attributeName, Object attributeValue) {
    Optional.ofNullable(attributeValue)
  ...
  private static final Map<SearchType, Function<SearchFilter, String>> keywordFormatterMap =
      Map.of(
          SearchType.ExactMatch,
              (SearchFilter searchFilter) -> encloseInQuotes(searchFilter.getSearch()),
          SearchType.AllKeywords,
              (SearchFilter searchFilter) ->
                  Stream.of(splitBySpace(searchFilter))
                      .map(ProjectQueryBuilder::encloseInQuotes)
                      .collect(Collectors.joining(" ")));

  public static String formatSearchString(SearchFilter searchFilter) {
