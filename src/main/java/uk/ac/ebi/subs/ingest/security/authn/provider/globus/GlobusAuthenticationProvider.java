...
public class GlobusAuthenticationProvider implements AuthenticationProvider {
  ...
  @Autowired
  public GlobusAuthenticationProvider(
      WebClient.Builder builder, AccountRepository accountRepository) {
    this.webClient =
        builder
    ...
    log.info(
        "[GlobusAuth] GlobusAuthenticationProvider initialised with baseUrl=https://auth.globus.org");
  }

  @Override
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    log.debug(
        "[GlobusAuth] authenticate() entered with {}",
    ...
    log.debug(
        "[GlobusAuth] UserInfo from Globus: sub={}, email={}, name={}, preferredUsername={}",
    ...
    if (userInfo.getSubjectId() == null) {
      ...
      throw new AuthenticationServiceException(
          "Invalid Globus user information (missing subjectId)");
    }
    ...
    log.debug(
        "[GlobusAuth] Resolved displayName='{}' for subjectId={}",
        displayName,
        userInfo.getSubjectId());
    ...
    log.info(
        "[GlobusAuth] Successfully authenticated Globus user sub={}, displayName={}, accountId={}",
        userInfo.getSubjectId(),
        displayName,
        account.getId());
  ...
  /**
   * Load an existing Account for this Globus subject, or create + save one. providerReference is
   * used to store the Globus "sub".
   */
  private Account loadOrCreateAccount(UserInfo userInfo, String displayName) {
    ...
    if (existing != null) {
      ...
      if (displayName != null
        ...
        log.debug(
            "[GlobusAuth] Updated existing Account {} name to '{}'", existing.getId(), displayName);
      } else {
        log.debug(
            "[GlobusAuth] Reusing existing Account {} for sub={}", existing.getId(), globusSub);
      }
      return existing;
    }
  ...
  protected UserInfo fetchUserInfo(String accessToken) {
    ...
    log.debug(
        "[GlobusAuth] Calling /v2/oauth2/userinfo with access token (preview): {}", tokenPreview);
    ...
    try {
      ...
      if (e.getStatusCode() == HttpStatus.UNAUTHORIZED
        ...
        log.debug(
            "[GlobusAuth] /userinfo rejected token with status {} – probably not a Globus token; delegating",
            e.getStatusCode());
        return null;
      }
