...
public class ProjectServiceTest {
  ...
  class SubmissionEnvelopes {
    ...
    void getFromAllCopiesOfProjectsNoDuplicates() {
      ...
      // and:
      when(projectRepository.findByUuid(project1.getUuid()))
          .thenReturn(Stream.of(project1, project2, project3));
  ...
  class Deletion {
    ...
    void succeedForEmptyProject() throws Exception {
      ...
      // and:
      var persistentProjects =
          IntStream.range(0, 3).mapToObj(number -> new Project(null)).collect(toList());
      persistentProjects.forEach(
          persistentProject -> BeanUtils.copyProperties(project, persistentProject));
      doReturn(persistentProjects.stream()).when(projectRepository).findByUuid(project.getUuid());
    ...
    void failForProjectWithSubmissions() {
      ...
      doReturn(Stream.of(persistentEmptyProject, persistentNonEmptyProject))
          ...
          .findByUuid(project.getUuid());

      // expect:
      Assertions.assertThatThrownBy(() -> projectService.delete(project))
    ...
    void givenSuggestionCreatesProjectSuccessfully() {
      ...
      Schema projectSchema =
          new Schema(
              ...
              "project",
              "project",
              "mock.io/mock-schema-project");
      doReturn(projectSchema)
          .when(schemaService)
          .getLatestSchemaByEntityType(highLevelEntity, "project");
  ...
  class ProjectUpdate {
    ...
    void statusUpdatesAddsHistoryRecord() {
      ...
      verify(auditEntryService)
          .addAuditEntry(new AuditEntry(STATUS_UPDATED, any(), ELIGIBLE, project));
    }
  }
}
