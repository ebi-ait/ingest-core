...
public class ProjectFilterQueryBuilderTest {
  ...
  public void quoting_in_mongo_syntax_by_search_type(
      String searchTypeStr, String input, String expected) {
    SearchType searchType = searchTypeStr.equals("null") ? null : SearchType.valueOf(searchTypeStr);
    SearchFilter searchFilter = SearchFilter.builder().search(input).searchType(searchType).build();
    Assertions.assertThat(ProjectQueryBuilder.formatSearchString(searchFilter)).isEqualTo(expected);
  }
}
