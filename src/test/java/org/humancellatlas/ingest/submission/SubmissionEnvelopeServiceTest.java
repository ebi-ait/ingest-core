...
public class SubmissionEnvelopeServiceTest {
  @Autowired private SubmissionEnvelopeService service;
  ...
  @MockBean private SubmissionEnvelopeCreateHandler submissionEnvelopeCreateHandler;
  ...
  @MockBean private PatchRepository patchRepository;
  ...
  public void testDeleteSubmission() {
    ...
    Biomaterial testBiomaterial =
        new Biomaterial(Map.ofEntries(Map.entry("key", UUID.randomUUID())));
    ...
    Biomaterial testOutsideBiomaterial =
        new Biomaterial(Map.ofEntries(Map.entry("key", UUID.randomUUID())));
    ...
    when(projectRepository.findBySubmissionEnvelopesContains(any())).thenReturn(Stream.of(project));

    when(projectRepository.findBySupplementaryFilesContains(any())).thenReturn(Stream.of(project));

    // when
    when(processRepository.findBySubmissionEnvelope(submissionEnvelope))
        .thenReturn(Stream.of(testProcess));
    when(biomaterialRepository.findBySubmissionEnvelope(submissionEnvelope))
        .thenReturn(Stream.of(testBiomaterial));
    when(protocolRepository.findBySubmissionEnvelope(submissionEnvelope))
    ...
    verify(bundleManifestRepository)
        .deleteByEnvelopeUuid(submissionEnvelope.getUuid().getUuid().toString());
  ...
  class SubmitRequestTests {
    ...
    public void testGraphValidationErrorsCleared() {
      ...
      when(biomaterialRepository.findBySubmissionEnvelope(any()))
          .thenReturn(Stream.of(testBiomaterial));
      when(processRepository.findBySubmissionEnvelope(any())).thenReturn(Stream.of(testProcess));
    ...
    public void testSubmissionNoProject() {
      // given:
      when(projectRepository.findBySubmissionEnvelopesContains(any())).thenReturn(Stream.empty());
    ...
    @Test
    void testExportedEventUpdatesHistory() {
      // given
