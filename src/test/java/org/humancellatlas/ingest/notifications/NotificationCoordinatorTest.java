...
public class NotificationCoordinatorTest {
  ...
  private NotificationService mockNotificationService() {
    ...
    Answer<Notification> mockChangeStateFn = invocation -> {
    ...
    Mockito.doAnswer(mockChangeStateFn)
           .when(notificationService).changeState(any(Notification.class), any(NotificationState.class));
  ...
  public void testQueue() {
    List<Notification> testNotifications = List.of(generateTestNotification("testvalue1"),
                                                   generateTestNotification("testvalue2"));
    ...
    Mockito.doReturn(testNotifications.stream())
           .when(notificationService).getUnhandledNotifications();

    NotificationCoordinator notificationCoordinator = new NotificationCoordinator(Collections.emptyList(),
                                                                                  testInmemorySource,
                                                                                  notificationService);
    ...
    Assertions.assertThat(testInmemorySource.stream()
              ...
              .containsSequence(testNotifications.stream()
                                                 .map(n -> n.getChecksum().getValue())
  ...
  public void testProcess() {
    ...
    Mockito.doReturn(Stream.of(generateTestNotification("testvalue1"),
                               generateTestNotification("testvalue2")))
           .when(mockInmemorySource).stream();

    NotificationService notificationService = mockNotificationService();

    NotificationProcessor mockHappyNotificationProcessor = mock(NotificationProcessor.class);
    Mockito.doNothing()
           .when(mockHappyNotificationProcessor).handle(any(Notification.class));

    NotificationCoordinator notificationCoordinator = new NotificationCoordinator(List.of(mockHappyNotificationProcessor),
                                                                                  mockInmemorySource,
                                                                                  notificationService);
  ...
  public void testProcessingFailure() {
    ...
    Mockito.doThrow(new ProcessingException(""))
           .when(mockUnhappyNotificationProcessor).handle(any(Notification.class));
    Mockito.doReturn(true)
           .when(mockUnhappyNotificationProcessor).isEligible(any(Notification.class));

    NotificationCoordinator notificationCoordinator = new NotificationCoordinator(List.of(mockUnhappyNotificationProcessor),
                                                                                  mockInmemorySource,
                                                                                  notificationService);
  ...
  /**
   * Test behaviour when one processor succeeds, but another fails.
   * Expect an overall failure outcome.
   */
  @Test
  public void testMultipleProcessors_OneFailing() {
    ...
    Mockito.doNothing()
           .when(mockHappyNotificationProcessor).handle(any(Notification.class));
    Mockito.doReturn(true)
           .when(mockHappyNotificationProcessor).isEligible(any(Notification.class));
    ...
    NotificationCoordinator notificationCoordinator = new NotificationCoordinator(List.of(mockUnhappyNotificationProcessor,
                                                                                          mockHappyNotificationProcessor),
                                                                                  fakeSource,
                                                                                  notificationService);

    fakeSource.supply(List.of(generateTestNotification("testvalue1"),
                              generateTestNotification("testvalue2")));
    notificationCoordinator.process();
