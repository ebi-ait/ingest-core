...
import uk.ac.ebi.subs.ingest.config.ConvertersConfiguration;
...
class ProjectFilterTest {
  ...
  @MockBean private AuditEntryService auditEntryService;
  ...
  void test_raw_criteria() {
    Query query =
        new Query()
            .addCriteria(
                Criteria.where("content.project_core.project_title").regex("project1", "i"));
  ...
  void filter_by_wrangler() {
    ...
    SearchFilter searchFilter =
        SearchFilter.builder().primaryWrangler(this.project2.getPrimaryWrangler()).build();
  ...
  void filter_by_wrangling_priority() {
    ...
    // when
    SearchFilter searchFilter =
        SearchFilter.builder().wranglingPriority(this.project1.getWranglingPriority()).build();
  ...
  void filter_by_cell_count() {
    ...
    // when
    SearchFilter searchFilter =
        SearchFilter.builder().maxCellCount(project1.getCellCount()).minCellCount(0).build();
  ...
  void filter_by_data_access() {
    // given
    Project project4 = makeProject("project4");
    ((Map<String, Object>) project4.getContent())
        .put("dataAccess", new DataAccess(DataAccessTypes.MANAGED));
    this.mongoTemplate.save(project4);
    // when
    SearchFilter searchFilter = SearchFilter.builder().dataAccess(DataAccessTypes.MANAGED).build();
  ...
  void query_all_keywords__order_independent() {
    SearchFilter searchFilter =
        SearchFilter.builder().search("liver human").searchType(SearchType.AllKeywords).build();
  ...
  void query_any_keywords() {
    SearchFilter searchFilter =
        SearchFilter.builder().search("liver mouse").searchType(SearchType.AnyKeyword).build();
  ...
  void query_exact_phrase__correct_order() {
    SearchFilter searchFilter =
        SearchFilter.builder().search("human liver").searchType(SearchType.ExactMatch).build();
  ...
  void query_exact_phrase__reverse_order() {
    SearchFilter searchFilter =
        SearchFilter.builder().search("liver human").searchType(SearchType.ExactMatch).build();
  ...
  void lookup_by_uuid() {
    ...
    SearchFilter searchFilter = SearchFilter.builder().search(uuidString).build();
