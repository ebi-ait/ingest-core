...
public class FileControllerTest {
  ...
  @Test
  public void testLinkFileAsInputToProcessesUsingPutMethodWithManyProcessesInPayload()
      throws Exception {
    ...
    webApp
        .perform(
            put("/files/{fileId}/inputToProcesses/", file.getId())
  ...
  @Test
  public void testLinkFileAsInputToMultipleProcessesUsingPostMethodWithManyProcessesInPayload()
      throws Exception {
    // when
    webApp
        .perform(
            post("/files/{fileId}/inputToProcesses/", file.getId())
                .contentType("text/uri-list")
                .content(
                    uriBuilder.build().toUriString()
                        + "/processes/"
  ...
  @Test
  public void testLinkFileAsInputToProcessesUsingPostMethodWithOneProcessInPayload()
      throws Exception {
    // when
    webApp
        .perform(
  ...
  @Test
  public void testLinkFileAsDerivedByProcessesUsingPostMethodWithOneProcessInPayload()
      throws Exception {
    // when
    webApp
        .perform(
            post("/files/{fileId}/derivedByProcesses/", file.getId())
                .contentType("text/uri-list")
                .content(uriBuilder.build().toUriString() + "/processes/" + process1.getId()))
        .andExpect(status().isOk());
    TestingHelper.resetTestingSecurityContext();
  ...
  @Test
  public void testLinkFileAsDerivedByProcessesUsingPostMethodWithManyProcessesInPayload()
      throws Exception {
    // when
    webApp
        .perform(
  ...
  @Test
  public void testLinkFileAsDerivedByProcessesUsingPutMethodWithManyProcessesInPayload()
      throws Exception {
    // given
    ...
    webApp
        .perform(
            put("/files/{fileId}/derivedByProcesses/", file.getId())
                .contentType("text/uri-list")
                .content(
                    uriBuilder.build().toUriString()
                        + "/processes/"
  ...
  public void testUnlinkFileAsDerivedByProcesses() throws Exception {
    ...
    webApp
        .perform(
            delete(
                "/files/{fileId}/derivedByProcesses/{processId}", file.getId(), process1.getId()))
  ...
  public void testUnlinkFileAsInputToProcesses() throws Exception {
    ...
    webApp
        .perform(
            delete("/files/{fileId}/inputToProcesses/{processId}", file.getId(), process1.getId()))
        .andExpect(status().isNoContent());
    TestingHelper.resetTestingSecurityContext();
  ...
  public void testValidationJobPatch() throws Exception {
    ...
    // when:
    String patch =
        "{ \"validationJob\": { \"validationReport\": { \"validationState\": \"Valid\" }}}";

    MvcResult result =
        webApp
            .perform(
                patch("/files/{id}", file.getId())
    ...
    assertThat(file.getValidationJob().getValidationReport().getValidationState())
  ...
  private ObjectNode createPayloadhNoDataFileUuid() {
    ...
    newFilePayload.put("fileName", "test-file").put("fileContentType", "text/plain");
    return newFilePayload;
  }
}
