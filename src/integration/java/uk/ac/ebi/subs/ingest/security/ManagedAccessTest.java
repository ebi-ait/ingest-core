...
import static uk.ac.ebi.subs.ingest.TestingHelper.resetTestingSecurityContext;
...
public class ManagedAccessTest {
  ...
  public void setupTestData() throws Exception {
    ...
    projects.stream()
        ...
        .forEach(
            p -> {
              try {
                webApp
                    .perform(post("/projects").contentType(MediaType.APPLICATION_JSON).content(p))
    ...
    Stream.of("a", "b", "c")
        ...
        .forEach(
            uuidString ->
                addMetadataToProjectByProjectUuid(
                    uuidString,
  ...
  public void testDataAccessTypeFieldDeserialization() {
    projectRepository
        .findByUuid(new Uuid(TestDataHelper.makeUuid("a")))
        .forEach(
            p ->
                Assertions.assertThat(p.getContent())
  ...
  class MetadataFromProjectAccessControl {
    ...
    @WithMockUser(
        username = "alice",
        roles = {"CONTRIBUTOR", "access_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"})
    public void userOnProjectAList_CanSeeProjectMetadata(String metadataTypePlural)
        throws Exception {
      String projectMetadataUrl =
          getProjectMetadataUrl(metadataTypePlural, TestDataHelper.makeUuid("a"));

      webApp.perform(get(projectMetadataUrl)).andExpect(status().isOk());
    }
    ...
    @WithMockUser(
        username = "alice",
        roles = {"CONTRIBUTOR", "access_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"})
    public void userOnProjectAList_CanSeeOpenProjectMetadata(String metadataTypePlural)
        throws Exception {
      String openAccessProjectMetadataUrl =
          getProjectMetadataUrl(metadataTypePlural, TestDataHelper.makeUuid("c"));

      webApp
          .perform(get(openAccessProjectMetadataUrl).contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk());
    }
    ...
    public void userNotOnProjectAList_CannotSeeMetadata(String metadataTypePlural)
        throws Exception {
      String projectMetadataUrl =
          getProjectMetadataUrl(metadataTypePlural, TestDataHelper.makeUuid("a"));

      webApp.perform(get(projectMetadataUrl)).andExpect(status().isForbidden());
    }
  }
  ...
  class ProjectAccessControl {
    ...
    public void userOnProjectAListCanSeeAllProjects() throws Exception {
      ...
      webApp.perform(get("/projects")).andExpect(jsonPath("$.page.totalElements").value("3"));
    }
  ...
  private void addMetadataToProjectByProjectUuid(
      String uuidString, List<Class<? extends MetadataDocument>> metadataTypes) {
    Project project = projectRepository.findByUuid(new Uuid(uuidString)).findFirst().get();
    try {
      final String submissionUrl = createSubmissionAndGetUrl();
      linkSubmissionToProject(project, submissionUrl);
      metadataTypes.forEach(
          metadataType -> {
            try {
              ...
              String submissionMetadataDocumentsUrl =
                  buildSubmissionMetadataUrl(submissionUrl, lowerCaseMetadataType);
              Map documentAsMap =
                  new ObjectToMapConverter(objectMapper)
                      .asMap(metadataDocument, List.of("contentLastModified"));
              webApp
                  .perform(
                      post(submissionMetadataDocumentsUrl)
                          .contentType(MediaType.APPLICATION_JSON)
                          .content(TestDataHelper.mapAsJsonString(documentAsMap)))
                  .andExpect(status().isAccepted());
              resetTestingSecurityContext();
            } catch (Exception e) {
              throw new RuntimeException(
                  "problem crating metadata document " + metadataType.getSimpleName(), e);
            }
          });
    } catch (Exception e) {
      throw new RuntimeException(e);
  ...
  private static String buildSubmissionMetadataUrl(
      String submissionUrl, String lowerCaseMetadataType) {
    return submissionUrl
        + "/"
        + lowerCaseMetadataType
        + (lowerCaseMetadataType.endsWith("s") ? "es" : "s");
  }
  ...
  private static void setDocumentProperties(String uuidString, MetadataDocument metadataDocument) {
    ...
    Map<String, Map<String, String>> content =
        Map.of(
            lowerCaseMetadataType + "_core",
            Map.of(
                lowerCaseMetadataType + "_name",
                lowerCaseMetadataType + " in project " + uuidString));
    metadataDocument.setContent(content);
    metadataDocument.setUuid(Uuid.newUuid());
  }

  @NotNull
  private static MetadataDocument newMetadataInstance(
  ...
  @NotNull
  private ResultActions linkSubmissionToProject(Project project, String submissionUrl)
      throws Exception {
    return webApp
        .perform(
  ...
  private String createSubmissionAndGetUrl() throws Exception {
    return webApp
        .perform(post("/submissionEnvelopes").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  class MetadataRepositoryAccessControl {
    ...
    @WithMockUser(
        username = "alice",
        roles = {"CONTRIBUTOR", "access_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"})
    public void userOnProjectAList_CanSeeOnlyOpenAndProjectAMetadata(String metadataTypePlural)
        throws Exception {
      String metadataCollectionUrl = "/" + metadataTypePlural;
      webApp
          .perform(get(metadataCollectionUrl))
          .andExpect(status().isOk())
  ...
  class MetadataFromSubmissionAccessControl {
    @ParameterizedTest
    @MethodSource("uk.ac.ebi.subs.ingest.security.SecurityTest#metadataTypesWithProject")
    @WithMockUser(
        username = "alice",
        roles = {"CONTRIBUTOR", "access_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"})
    public void userOnProjectAList_CanSeeSubmissionMetadata(String metadataTypePlural)
        throws Exception {
      String submissionMetadataUrl =
          getSubmissionMetadataUrl(metadataTypePlural, TestDataHelper.makeUuid("a"));
      webApp.perform(get(submissionMetadataUrl)).andExpect(status().isOk());
    }
  }
  ...
  private String getSubmissionMetadataUrl(String metadataTypePlural, String uuid) {
    return projectRepository
        ...
        .map(
            submissionId ->
                String.format("/submissionEnvelopes/%s/%s", submissionId, metadataTypePlural))
        .findFirst()
        .get();
  }
  ...
  class ServiceUserAccessControl {
    ...
    public void canCreateSubmissionManifest() throws Exception {
      ...
      webApp
          .perform(
              post(submissionUrl + "/submissionManifest")
                  .content("{}")
                  .contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isAccepted());
    }
    ...
    public void canGetSubmissionManifest() throws Exception {
      ...
      webApp
          ...
          .andExpect(status().isAccepted());
      webApp
          .perform(get(submissionUrl + "/submissionManifest").accept(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk());
    }
  }
}
