...
public class BiomaterialControllerTest {
  ...
  @MockBean ValidationStateChangeService validationStateChangeService;
  ...
  @Autowired BiomaterialService biomaterialService;
  ...
  public void newBiomaterialInSubmissionLinksToSubmissionAndProject() throws Exception {
    ...
    MvcResult allBiomaterialsResult =
        webApp
            .perform(get("/biomaterials").accept(MediaType.APPLICATION_JSON))
  ...
  private void verifyBiomaterial(MvcResult allBiomaterialsResult) throws Exception {
    webApp
        .perform(
            get(
                    "/biomaterials/search/findBySubmissionEnvelope?submissionEnvelope=http://localhost/submissionEnvelopes/{id}",
    ...
    Resources<Resource<Biomaterial>> halResponse =
        objectMapper.readValue(content, new TypeReference<>() {});

    halResponse.getContent().stream()
        .map(r -> r.getContent())
        .forEach(
            newBiomaterial -> {
              assertThat(newBiomaterial.getSubmissionEnvelope().getId())
                  .isEqualTo(submissionEnvelope.getId());
              assertThat(newBiomaterial.getProject().getId()).isEqualTo(project.getId());
              assertThat(newBiomaterial.getProjects()).hasSize(1);
              assertThat(newBiomaterial.getProjects().stream().findFirst().get().getId())
  ...
  @Test
  public void testLinkBiomaterialAsInputToProcessesUsingPostMethodWithManyProcessesInPayload()
      throws Exception {
    // when
    webApp
        .perform(
            post("/biomaterials/{id}/inputToProcesses/", biomaterial.getId())
  ...
  @Test
  public void testLinkBiomaterialAsInputToProcessesUsingPutMethodWithManyProcessesInPayload()
      throws Exception {
    // given
    ...
    webApp
        .perform(
            put("/biomaterials/{id}/inputToProcesses/", biomaterial.getId())
                .contentType("text/uri-list")
                .content(
                    uriBuilder.build().toUriString()
                        + "/processes/"
  ...
  @Test
  public void testLinkBiomaterialAsInputToProcessesUsingPostMethodWithOneProcessInPayload()
      throws Exception {
    // when
    webApp
        .perform(
  ...
  @Test
  public void testLinkBiomaterialAsDerivedByProcessesUsingPostMethodWithManyProcessesInPayload()
      throws Exception {
    // when
    webApp
        .perform(
            post("/biomaterials/{id}/derivedByProcesses/", biomaterial.getId())
                .contentType("text/uri-list")
                .content(
                    uriBuilder.build().toUriString()
                        + "/processes/"
  ...
  @Test
  public void testLinkBiomaterialAsDerivedByProcessesUsingPutMethodWithManyProcessesInPayload()
      throws Exception {
    // given
    ...
    webApp
        .perform(
            put("/biomaterials/{id}/derivedByProcesses/", biomaterial.getId())
                .contentType("text/uri-list")
                .content(
                    uriBuilder.build().toUriString()
                        + "/processes/"
  ...
  @WithMockUser(
      username = "alice",
      roles = {"WRANGLER"})
  public void testLinkBiomaterialAsDerivedByProcessesUsingPostMethodWithOneProcessInPayload()
      throws Exception {
    // when
    webApp
        .perform(
  ...
  public void testUnlinkBiomaterialAsInputToProcesses() throws Exception {
    ...
    webApp
        .perform(
            delete(
                "/biomaterials/{id}/inputToProcesses/{processId}",
                biomaterial.getId(),
                process1.getId()))
  ...
  public void testUnlinkBiomaterialAsDerivedByProcesses() throws Exception {
    ...
    webApp
        .perform(
            delete(
                "/biomaterials/{id}/derivedByProcesses/{processId}",
                biomaterial.getId(),
                process1.getId()))
        .andExpect(status().isNoContent());
    TestingHelper.resetTestingSecurityContext();
  ...
  private void verifyThatValidationStateChangedToDraftWhenGraphValid(MetadataDocument... values) {
    Arrays.stream(values)
        .forEach(
            value ->
                verify(validationStateChangeService, times(1))
